...
st.title("HB's Softs & Ags Dashboard")

# -------------------------
# Common softs & ags (Yahoo Finance futures tickers)
# -------------------------
SOFTS = {
    # 1) Classic Softs (ICE)
    "Coffee (Arabica)": "KC=F",
    ...
    # 2) Grains
    "Corn": "ZC=F",
    "Wheat (Chicago SRW)": "ZW=F",
    "Wheat (Kansas HRW)": "KE=F",
    "Oats": "ZO=F",
    "Rough Rice": "ZR=F",

    # 3) Oilseeds & By-products
    "Soybeans": "ZS=F",
    "Soybean Meal": "ZM=F",
    "Soybean Oil": "ZL=F",
    "Canola (ICE)": "RS=F",
...
# Top selector row
# -------------------------
st.subheader("Select softs/ag contracts")
col_sel, col_custom = st.columns([2, 1])

with col_sel:
    selected_names = st.multiselect(
        "Contracts (toggle on/off):",
        options=list(SOFTS.keys()),
        default=["Corn"],
        help="Select one or more to plot."
    )
...
selected_tickers = [SOFTS[n] for n in selected_names] + custom_tickers
...
data = data.rename(columns=rename_map).dropna(how="all")
# Restrict to selected labels that actually downloaded
present_labels = [lbl for lbl in selected_labels if lbl in data.columns]
data = data[present_labels].dropna(how="all")
...
with col1:
    ...
    fig = px.line(plot_df, x=plot_df.index, y=plot_df.columns,
                  labels={"value": y_axis_label, "variable": "Contract", "index": "Date"})
...
with col2:
    ...
    if returns.empty:
        ...
        fig = px.line(returns, x=returns.index, y=returns.columns,
                      labels={"value": "Returns", "variable": "Contract", "index": "Date"})
        st.plotly_chart(fig, use_container_width=True)
    ...
    if data.shape[1] == 0:
        ...
        if vol.dropna(how="all").empty:
            ...
            fig = px.line(vol, x=vol.index, y=vol.columns,
                          labels={"value": "Volatility (Annualised)", "variable": "Contract", "index": "Date"})
            st.plotly_chart(fig, use_container_width=True)
